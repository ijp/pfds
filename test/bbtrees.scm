  ...
  (define-test-case bbtrees bbtree-update ()
    (let ([bb (alist->bbtree '(("foo" . 10) ("bar" . 12)) string<?)]
      ...
      (test-case bbtree-update ()
        (test-eqv 11 (bbtree-ref (bbtree-update bb "foo" add1 0) "foo"))
        (test-eqv 13 (bbtree-ref (bbtree-update bb "bar" add1 0) "bar"))
        (test-eqv  1 (bbtree-ref (bbtree-update bb "baz" add1 0) "baz")))))
