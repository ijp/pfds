(library (test sets)
  ...
  (define-test-case sets set-basics
    (let ([empty (make-set string<?)]
      ...
      (test-case set-basics ()
        (assert-predicate set? empty)
        (assert-predicate set? set)
        (assert-eqv 0 (set-size empty))
        (assert-eqv 5 (set-size set))
        (assert-eqv #f (set-member? empty "foo"))
        (assert-eqv #t (set-member? (set-insert empty "foo") "foo"))
        (assert-eqv #t (set-member? set "foo"))
        (assert-eqv #f (set-member? (set-remove set "foo") "foo"))
        (assert-no-raise (set-remove empty "anything"))
        (assert-no-raise (set-insert set "quux"))
        (assert-eqv (set-size (set-insert empty "foo"))
                  (set-size (set-insert (set-insert empty "foo") "foo")))
        (assert-eqv (set-size (set-remove set "foo"))
  ...
  (define-test-case sets set-equality
    (let* ([empty (make-set string<?)]
      ...
      (test-case set-equality ()
        ...
        (assert-not (exists (lambda (x) (proper-subset? x x)) sets))
  ...
  (define-test-case sets set-conversion ()
    (assert-eqv '() (set->list (make-set <)))
    (assert-eqv 0 (set-size (list->set '() <)))
    (assert-equal (string->list "abcdefghijklmno")
                (list-sort char<?
                           (set->list
                            (list->set (string->list "abcdefghijklmno") char<?))))
    (assert-equal '(0) (set->list (fold-left set-insert (make-set <) '(0 0 0 0)))))
  
  (define-test-case sets set-iterators ()
    (assert-eqv 0 (set-fold + 0 (list->set '() <)))
    (assert-eqv 84 (set-fold + 0 (list->set '(3 12 62 7) <)))
    (assert-eqv 499968 (set-fold * 1 (list->set '(3 12 62 7 8 4) <))))
